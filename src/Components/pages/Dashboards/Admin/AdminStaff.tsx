...
import { 
  ...
  EnvelopeIcon, ClockIcon, SparklesIcon,
  PlusIcon, XMarkIcon, IdentificationIcon
...
export const AdminStaff = () => {
  ...
  // Opciones estáticas para el turno
  const turnos = ['Matutino (8:00 AM - 2:00 PM)', 'Vespertino (2:00 PM - 8:00 PM)', 'Turno Completo', 'Fines de Semana'];
  ...
  const initialForm = {
    ...
    telefono: '',
    email: '',
    password: '', // <-- Para la creación de cuenta
    turno: '',
    activo: true
  };
  ...
  const fetchStaff = async () => {
    ...
    try {
      ...
      console.error("Error al cargar el personal administrativo:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveStaff = async (e: React.FormEvent) => {
    ...
    if (!formData.nombres || !formData.apellidos || !formData.turno || (!formData.id && !formData.password)) {
      Swal.fire('Faltan Datos', 'Revisa los campos obligatorios. Recuerda que el personal nuevo necesita una contraseña temporal.', 'warning');
      return;
    }
    ...
    try {
      if (formData.id) {
        ...
        if (error) throw error;
        Swal.fire('¡Actualizado!', 'Los datos se guardaron al chaz.', 'success');
      } else {
        // --- MODO NUEVO: LLAMADA A LA FUNCIÓN RPC ---
        const { error } = await supabase.rpc('registrar_staff_completo', {
        ...
        if (error) throw error;
        Swal.fire('¡Personal Registrado!', 'Cuenta de acceso generada con éxito.', 'success');
      }
          ...
          <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-8 mb-16">
            <div>
              ...
              <h1 className="text-5xl lg:text-7xl font-serif italic tracking-tighter">
                Recepción <span className="text-[#F4B6B6]">& Staff</span>
              </h1>
            </div>
            ...
            {filteredStaff.map((persona) => (
              ...
              >
                ...
                <div className="absolute top-8 right-8 flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${persona.activo ? 'bg-[#629483]' : 'bg-red-400'} animate-pulse`} />
                ...
                <div className="flex flex-col items-center text-center mt-4">
                  ...
                  <div className="relative mb-6">
                    ...
                    <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-[#4A3737] text-white px-4 py-1.5 rounded-full shadow-lg flex items-center gap-1">
                  ...
                  <h3 className="text-3xl font-serif italic text-[#4A3737] mt-4 mb-8">
                  ...
                  <div className="w-full bg-[#F8F7F4] rounded-3xl p-5 mb-8 flex flex-col gap-3 text-left group-hover:bg-white transition-colors">
                    <div className="flex items-center gap-3">
                      <ClockIcon className="w-5 h-5 text-[#4A3737] shrink-0" />
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Turno Asignado</p>
                        <p className="text-xs font-bold text-[#4A3737]">{persona.turno || 'Por definir'}</p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-2xl w-full max-w-2xl border border-[#F4B6B6]/30 relative animate-fade-in overflow-y-auto max-h-[90vh]">
            ...
            <div className="mb-8">
              <h3 className="text-4xl font-serif italic text-[#4A3737]">
                {formData.id ? 'Editar' : 'Nuevo'} <span className="text-[#F4B6B6]">Personal</span>
              </h3>
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mt-2">
                {formData.id ? 'Modifica los datos del expediente' : 'Registra un nuevo miembro del equipo administrativo'}
              </p>
            </div>
            ...
            <form onSubmit={handleSaveStaff} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 ml-2">Turno Asignado *</label>
                  <select 
                    required 
                    name="turno" 
              ...
              {!formData.id && (
                <div className="grid grid-cols-1 gap-6">
                  <div>
                    ...
                    <input required type="text" name="password" value={formData.password} onChange={handleInputChange} placeholder="Ej. LuminousStaff2026" className="w-full bg-[#F8F7F4] border-none rounded-2xl py-4 px-6 text-[#4A3737] font-bold outline-none focus:ring-2 focus:ring-[#F4B6B6] transition-all" />
                    <p className="text-[9px] text-[#F4B6B6] font-bold mt-2 ml-2 tracking-widest uppercase">Dale esta contraseña a la recepcionista para que pueda ingresar.</p>
                  </div>
                </div>
              )}
              ...
              {formData.id && (
                <div className="flex items-center gap-3 bg-[#F8F7F4] p-4 rounded-2xl">
                  ...
                  />
                  <div>
                    <p className="text-sm font-bold text-[#4A3737]">Acceso al Sistema</p>
                    <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Si lo desactivas, no podrán iniciar sesión ni gestionar citas.</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-4 pt-6 mt-6 border-t border-gray-100">
                ...
                >
                  {formData.id ? 'Guardar Cambios' : 'Registrar Staff y Crear Cuenta'}
                </button>
              </div>
            </form>
